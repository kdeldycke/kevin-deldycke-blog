...
1. Go to `Settings` > `Configure Konqueror...` menu.

1. Then go to `AdBlock Filters` panel (the one on the screenshot).

1. Check both `Enable filters` and `Hide filtered images` options.

1. Use the `Import...` button to load the "Filterset.G" file you previoulsy downloaded

1. Enjoy ad-free web sites!
