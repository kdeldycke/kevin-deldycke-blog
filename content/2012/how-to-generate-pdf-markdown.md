...
![PDF page from pandoc where the long apt-get command runs straight off the right edge of the paper and is cut off]({attach}pandoc-non-wrapping-code-blocks.png)
...
![The same page from gimli, the long command wrapped onto a second line inside a grey code box that fits the margins]({attach}gimli-wrapping-code-blocks.png)
