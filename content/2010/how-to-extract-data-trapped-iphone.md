...
![iTunes summary page for a 14.46 GB iPhone on software 4.1, the progress bar at the top reading backing up as step 1 of 4, serial and phone number scribbled out]({attach}iphone-itunes-sync.png)

Then you can download and run the iPhone Backup Extractor app:

![iPhone Backup Extractor freshly launched with an empty application list, its Read Backups and Extract buttons at the bottom, over a Finder window showing the downloaded app]({attach}iphone-backup-extractor.png)
...
![Backup picker listing two snapshots of the same iPhone, one from September 15 and one from August 30, 2010]({attach}list-of-iphone-backups.png)
...
![Application list from the backup, showing Bloomberg, LinkedIn, Facebook and Flashlight, with the iOS Files entry selected below them]({attach}iphone-backup-content.png)

![Destination chooser pointed at the Documents folder, with an Extract Here button]({attach}iphone-backup-extraction-destination.png)

Then the extraction itself will take place:

![Small progress bar reporting the extraction of a photo from Media/PhotoData/103APPLE]({attach}iphone-backup-extraction.png)
...
![Finder on the extracted iOS Files folder, holding Library, Media, mobile and SystemConfiguration alongside the keychain backup and two SQLite databases]({attach}iphone-backup-extraction-content.png)
...
![Finder column view drilling from Media into DCIM and the 103APPLE folder, listing the camera roll as numbered IMG JPEG files]({attach}iphone-photo-location.png)

Most of other data are located in the `/iOS Files/Library/` folder. For
example here are voice messages:

![Finder column view of the Library folder, its Voicemail subfolder holding two .amr recordings, two plists and voicemail.db]({attach}iphone-voicemessages-location.png)
...
![SQLite Database Browser open dialog with voicemail.db selected, a 29 KB database document]({attach}sqlite-database-browser-opening.png)

![Database structure tab of voicemail.db, listing the voicemail table and its date and remote_uid indexes with their CREATE statements]({attach}iphone-voicemail-database-tables.png)

![Browse data tab showing the two rows of the voicemail table with columns for date, token, sender and duration, the personal fields blurred out]({attach}iphone-voicemail-table-content.png)

![File menu open on Export, offering to write the database to an SQL file or the table as a CSV file]({attach}sqlite-csv-table-export.png)
...
![iPhone Backup Extractor listing a single May 6, 2010 backup, reached through the symlinked backup folder]({attach}iphone-backup-extractor-from-old-backup.png)
